...
**Workflow tools** (multi-step orchestration for common end-to-end tasks):

| Name | Description | Key Inputs | Annotations |
|:-----|:------------|:-----------|:------------|
| `mailchimp_send_campaign` | Compose and send (or schedule/test) a campaign in one call: create the campaign, set HTML/plaintext/template content, run send-checklist, optionally send a test, then send or schedule. | `audienceId`, `subject`, `fromName`, `replyTo`, `content` (html \| plaintext \| templateId+mergeData), `segmentId?`, `mode` (`draft`\|`test`\|`send`\|`schedule`), `scheduleTime?`, `testEmails?` | none (write, non-idempotent) |
| `mailchimp_upsert_subscriber` | Add or update a subscriber with status, merge fields, tags, interests, and optional note in a single idempotent call. Uses PUT /members/{hash} + tags sync. | `audienceId`, `email`, `status` (`subscribed`\|`unsubscribed`\|`cleaned`\|`pending`\|`transactional`), `mergeFields?`, `tags?`, `interests?`, `note?`, `vip?`, `language?` | `idempotentHint: true` |
...
| `mailchimp_find_subscriber` | Search for a subscriber by email across one or all audiences. Wraps `/search-members` and enriches with member detail + tags. | `email`, `audienceId?` | `readOnlyHint: true` |
...
**Primitive tools** (fine-grained CRUD, consolidated by noun via `operation` enum):
...
| `mailchimp_audiences` | Audience (list) CRUD and audience-level analytics. Free plan is capped at 1 audience. | `list`, `get`, `create`, `update`, `delete`, `list-activity`, `list-growth`, `list-clients`, `list-abuse-reports`, `list-locations` | mixed |
| `mailchimp_members` | Subscriber CRUD (minus upsert — use workflow tool), tags, notes, activity, events, goals. | `list`, `get`, `update`, `archive`, `delete-permanent`, `list-tags`, `set-tags`, `list-notes`, `add-note`, `update-note`, `delete-note`, `list-activity`, `list-events`, `list-goals` | mixed |
...
| `mailchimp_campaigns` | Campaign CRUD and non-send actions. Send/test/schedule live in `mailchimp_send_campaign`. | `list`, `get`, `create`, `update`, `delete`, `replicate`, `get-content`, `set-content`, `get-checklist`, `cancel-send`, `create-resend`, `pause-rss`, `resume-rss` | mixed |
| `mailchimp_reports` | Individual report slices (use `mailchimp_campaign_report` for the common digest view). | `list`, `get`, `list-click-details`, `get-click-link`, `list-open-details`, `list-email-activity`, `list-domain-performance`, `list-sent-to`, `list-unsubscribed`, `list-locations`, `list-eepurl`, `list-advice`, `list-abuse-reports` | `readOnlyHint: true` |
...
- Connected sites, verified domains, batch webhooks, account exports, authorized apps — admin-tier or infra-level, defer until demand surfaces.
...
- Support all Marketing API operations that work on the **Free plan** across: audiences, members, segments, merge fields, interests, campaigns (regular/plaintext/rss), templates, reports, folders, file manager, landing pages, search.
- Expose workflow tools for the six highest-frequency tasks (send campaign, upsert/import/find subscribers, campaign report digest, audience overview) — primitive tools for everything else.
- Respect Mailchimp rate limits: ≤10 concurrent requests per account; backoff on 429 with `Retry-After` honored.
- 120-second upstream timeout; fail fast with `ServiceUnavailable` on timeout or 5xx.
...
|:--------|:------|:--------|
| `MailchimpService` | Mailchimp Marketing API v3.0 (`https://{dc}.api.mailchimp.com/3.0/`). Handles DC resolution from key, basic auth header, retry/backoff via `withRetry`, 429 + `Retry-After`, timeout → `ServiceUnavailable`, 404 → `NotFound`, upstream validation → `ValidationError`. | All tools except the ones without any upstream call (none — every tool makes at least one request). |

Service exposes typed methods per endpoint group (e.g. `audiences.list`, `audiences.get`, `members.upsert`, `campaigns.create`, `reports.get`, `reports.clickDetails`, ...). Each method calls `withRetry` with a calibrated backoff (base 500ms for general errors, base 2s when upstream returns 429 without a `Retry-After`, respect header when present). Methods consuming array-ID batch endpoints (e.g. `members.getMany` via `?unique_email_id`) surface missing IDs as explicit `failed` entries rather than silent omissions.
...
| Timeout | 60s per request via `fetchWithTimeout` (API claims 120s but 60s is sufficient for free-plan-sized workloads). |
...
| `MAILCHIMP_BASE_URL` | no | Override the API base URL for mock servers / tests. Defaults to `https://{dc}.api.mailchimp.com/3.0` where `{dc}` is parsed from the API key. |
| `MAILCHIMP_TIMEOUT_MS` | no | Per-request timeout in ms. Default `60000`. |
| `MAILCHIMP_MAX_RETRIES` | no | Max retry attempts for transient failures. Default `3`. |
...
|:---------------|:------|:-----------|:------|
| `lists` (69) | ✅ | `mailchimp_audiences`, `mailchimp_members`, `mailchimp_segments`, `mailchimp_merge_fields`, `mailchimp_interests`, `mailchimp_upsert_subscriber`, `mailchimp_import_subscribers`, `mailchimp_find_subscriber`, `mailchimp_audience_overview` | Free capped at 1 audience. Legacy `/lists/**` path. |
| `campaigns` (22) | ✅ | `mailchimp_campaigns`, `mailchimp_send_campaign` | Regular/plaintext/RSS only on free. Split/variate paid. Scheduling on free plan requires Standard on some accounts — surface 403 with hint. |
| `reports` (22) | ✅ | `mailchimp_reports`, `mailchimp_campaign_report` | Basic post-send reports free. |
...
| `batch-webhooks` (5) | ✅ | — | Infra-level. Defer. |
| `batches` (4) | ✅ | — | Generic batch operation runner. Defer — our workflow tools handle batches inline. |
...
| `ping` (1) | ✅ | `mailchimp_account` (`operation: ping`) | |
...
**v1 coverage:** 19 tools covering ~175 operations (~62% of the API). The remaining ~107 ops are in excluded resources (automations, ecommerce, surveys, etc.) that are either paid-gated, admin-tier, or niche enough to defer until demand emerges.

---

## Workflow Analysis

How the six workflow tools compose upstream calls — each replaces a multi-step agent sequence with a single tool call.
...
```
Output carries the campaign ID, send-checklist warnings (not just errors — agents may want to proceed), and resolved mode. Failures from the checklist classify as `ValidationError` with the list of blocking items.
...
```
Uses PUT to avoid the "already subscribed" race. `status_if_new` preserves existing status when updating. Tags are synced — provided tag set becomes the set (agents pass the full desired state, not deltas, to keep the tool declarative).

**`mailchimp_import_subscribers`** (1 call, batch):
```
POST   /lists/{id}                           → batch with members[] array
```
Wraps the native batch subscribe. Returns per-row `total_created`, `total_updated`, `error_count` plus per-error `email_address`, `error`, `error_code`. Caps input at 500 per call (Mailchimp hard limit).
...
**`mailchimp_campaign_report`** (4–6 calls, parallelized):
```
GET /reports/{id}
GET /reports/{id}/click-details?count=10&sort_field=total_clicks
GET /reports/{id}/open-details?count=10&sort_field=opens
GET /reports/{id}/locations?count=10
GET /reports/{id}/unsubscribed?count=10
```
All parallelized via `Promise.all`. Output is a single structured digest. Individual slices remain available via `mailchimp_reports` for agents that need one specific dimension in detail.
...
**Consolidate REST verbs into `operation` enum.** Mailchimp's REST surface has 69 `/lists/**` endpoints. Exposing one tool per endpoint would blow up the surface. Grouping by noun with `operation` enum (`list`, `get`, `create`, `update`, `delete`, ...) keeps the surface at 19 tools while preserving full CRUD access.

**No tool for audiences-beta.** The `/audiences/**` surface (omni-channel contacts, BETA) overlaps with `/lists/**` but offers fewer features on free. Expose `/lists` via `mailchimp_audiences` + `mailchimp_members`, skip `/audiences`. Revisit if it exits BETA with meaningfully different capabilities.

**Legacy `/lists` naming.** Mailchimp UI calls them "audiences" but the API calls them "lists". Tool names use the user-facing term (`mailchimp_audiences`, `mailchimp_audience_overview`). Internally the service maps to `/lists/**` — this is a domain mismatch agents shouldn't see.

**`send` split from `campaigns`.** Send/schedule/test are their own workflow tool because they're the moment consequences happen (emails go to real people). Keeping them distinct from CRUD makes destructive-hint annotations and future approval flows (`elicit` before send?) cleaner.

**Tags as declarative state in `upsert_subscriber`.** Mailchimp's tag API has add/remove endpoints but not "set". The workflow tool reads current tags, computes delta, issues the right add/remove calls — agents pass the desired set, not operations. Trades one extra GET for a much cleaner interface.
...
- **Rate limit is per-account-concurrent, not per-period.** 10 simultaneous requests. Workflow tools that parallelize (e.g. `campaign_report`) cap internal concurrency to avoid contention with other agent calls.
...
- **Timeout:** Mailchimp-side 120s. We set client-side 60s.
...
- **Reference spec:** `docs/reference/mailchimp-openapi.json` (bundled Swagger 2.0, 10 MB, pulled from [Mailchimp codegen repo](https://github.com/mailchimp/mailchimp-client-lib-codegen)).
...
1. **Config** — `src/config/server-config.ts`. Zod schema, DC extraction from key suffix, optional overrides. Tests.
2. **Service types** — `src/services/mailchimp/types.ts`. Domain types (`Audience`, `Member`, `Campaign`, `Report`, `MergeField`, etc.) mapped from upstream responses. Prefer narrow types over `unknown`.
3. **MailchimpService** — `src/services/mailchimp/mailchimp-service.ts`. `fetchWithTimeout` + `withRetry`, basic-auth, error classification (401 → Unauthorized, 403-paid → Forbidden+hint, 404 → NotFound, 429 → RateLimited, 5xx → ServiceUnavailable). Init/accessor pattern registered in `setup()`.
4. **Remove echo scaffolds** — delete `echo.tool.ts`, `echo-app.app-tool.ts`, `echo.resource.ts`, `echo-app-ui.app-resource.ts`, `echo.prompt.ts`; update `src/index.ts`; delete tests under `tests/`.
5. **`mailchimp_account`** — ping/info/activity-feed. Smallest tool, validates the whole auth + service plumbing end-to-end.
6. **`mailchimp_audiences`** + **`mailchimp_audience_overview`** — list/get/CRUD + the digest workflow tool.
7. **`mailchimp_members`** + **`mailchimp_upsert_subscriber`** + **`mailchimp_find_subscriber`** + **`mailchimp_import_subscribers`** — the subscriber surface.
8. **`mailchimp_segments`, `mailchimp_merge_fields`, `mailchimp_interests`** — audience config tools.
9. **`mailchimp_campaigns`** + **`mailchimp_send_campaign`** — campaign CRUD and the send workflow.
10. **`mailchimp_reports`** + **`mailchimp_campaign_report`** — reports + digest.
...
14. **Field-test** with a real API key via `skills/field-test`.
15. **`polish-docs-meta`** — README, CHANGELOG, version sync, server.json description refresh.

Estimated effort: small service + 19 tools + 4 resources ≈ 30–40 files, with tests. 2–3 focused sessions.
